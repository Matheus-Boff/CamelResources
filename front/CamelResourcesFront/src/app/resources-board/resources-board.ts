...
@Component({
  ...
  standalone: true,
  imports: [NgFor, NgSwitch, NgSwitchCase, MatSnackBarModule],
...
export class ResourcesBoard implements OnInit, OnChanges {
  ...
  onReservarNote(recurso: any) {
    ...
    if (idUsuario) {
      this.alocacoesService
        .addAlocacaoNote(idUsuario, recurso.id.toString(), this.selectedDate)
        .subscribe({
          next: () => {
            this.snackbarService.show('Notebook reservado com sucesso!');
            this.loadResources();
          ...
          error: (err) => {
            ...
            this.snackbarService.show('Erro ao realizar reserva');
          },
        });
    }
  }
  ...
  onReservarSala(recurso: any) {
    ...
    if (idUsuario) {
      this.alocacoesService
        .addAlocacaoSala(idUsuario, recurso.id.toString(), this.selectedDate)
        .subscribe({
          next: () => {
  ...
  onReservarLab(recurso: any) {
    ...
    if (idUsuario) {
      this.alocacoesService
        .addAlocacaoLab(idUsuario, recurso.id.toString(), this.selectedDate)
        .subscribe({
          next: () => {
  ...
  loadResources() {
    ...
    if (idUsuario) {
      this.alocacoesService.getAlocacoesById(idUsuario).subscribe((allocations) => {
        const userAllocationsOnDate = allocations.filter(
          (a) => new Date(a.dataAlocacao).toDateString() === this.selectedDate.toDateString()
        );
